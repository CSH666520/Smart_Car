...
//    double d_yaw   = abs((IMU.angle.yaw_a-IMU.angle.yaw_a_old)/(float)delta_t);
    double d_gx_r = abs1((IMU.data.ax-IMU.data.ax_old)/(float)delta_t);
    double d_gy_r = abs1((IMU.data.ay-IMU.data.ay_old)/(float)delta_t);
    double d_gz_r = abs1((IMU.data.az-IMU.data.az_old)/(float)delta_t);
    //d_yaw大约在0~1之间
...
void Get_New_Angle(void)
{       
   if( Error_Angle.time == 1000 && Error_Angle.flag == 0)
   {          
       Error_Angle.time = 0;
       Error_Angle.add= IMU.angle.yaw_a;
       Error_Angle.k = Error_Angle.add/ 1000;
       Error_Angle.flag= 1;
   }
}
